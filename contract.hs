...
data ContractInfo = ContractInfo
    { policyAnimaliens :: !CurrencySymbol
    , policyBid :: !CurrencySymbol
    , prefixAnimaliens :: !BuiltinByteString
    , prefixAnimaliens :: !BuiltinByteString
...
tradeValidate contractInfo@ContractInfo{..} tradeDatum tradeAction context = case tradeDatum of
    ...
    Bid details@TradeDetails{..} -> case tradeAction of
        ...
        Sell ->
            ...
            containsPolicyBidNFT scriptOutputValue budId && -- expected correct bidPolicy NFT
            containsAnimaliensNFT (valuePaidTo txInfo tradeOwner) budId && -- expected bidder to be paid
    ...
    Offer TradeDetails{..} -> case tradeAction of
        Buy ->
            traceIfFalse "missing nft" (containsAnimaliensNFT (valuePaidTo txInfo signer) budId) && -- expected buyer to be paid
    ...
    where
        ...
        containsPolicyBidNFT v tn = valueOf v policyBid (TokenName (prefixAnimaliensBid <> tn)) >= 1

        containsAnimaliensNFT :: Value -> BuiltinByteString -> Bool
        containsAnimaliensNFT v tn = valueOf v policyAnimaliens (TokenName (prefixAnimaliens <> tn)) >= 1
        ...
        !scriptInputValue =
            ...
            in
                case xs of
                    ...
                    [v1, v2] ->
                        -- allow 2 script inputs if it's a combination of cancelling bid and buying OR cancelling offer and selling (same animaliens only)
                        case tradeDatum of
                            (Bid TradeDetails{..}) -> case (containsPolicyBidNFT v1 budId, containsAnimaliensNFT v2 budId) of
                                    (True, True) -> v1 -- expected script input 1 to contain bid token and script input 2 animaliens
                                    (False, False) -> v2 -- expected script input 2 to contain bid token and script input 1 animaliens
                            (Offer TradeDetails{..}) -> case (containsAnimaliensNFT v1 budId, containsPolicyBidNFT v2 budId) of
                                (True, True) -> v1 -- expected script input 2 to contain bid token and script input 1 animaliens
                                (False, False) -> v2 -- expected script input 1 to contain bid token and script input 2 animaliens
...
newContractInfo pid tokenPrefix teamPkh projectPkh communityPkh = ContractInfo
    { policyAnimaliens = pid
    , policyBid = "800df05a0cc6b6f0d28aaa1812135bd9eebfbf5e8e80fd47da9989eb"
    , prefixAnimaliens = tokenPrefix
    , prefixAnimaliensBid = "animaliensBid"
